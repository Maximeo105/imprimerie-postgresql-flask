...
create sequence NO_FINITION_SEQ start with 1 increment by 1;
...
-- Table des types de finition
CREATE TABLE TYPE_FINITION (
    NO_FINITION integer not null default nextval('NO_FINITION_SEQ'),
    NOM_FINITION varchar(50) not null,
    
    CONSTRAINT PK_TYPE_FINITION PRIMARY KEY (NO_FINITION),
    CONSTRAINT AK_TYPE_FINITION UNIQUE (NOM_FINITION)
);

-- Données initiales
INSERT INTO TYPE_FINITION (NOM_FINITION) VALUES 
    ('Plié'),
    ('Broché'),
    ('Laminé'),
    ('Perforé'),
...
alter table LIVRAISON
alter column NO_BON_LIVRAISON type varchar(100); 

alter table LIVRAISON_ARCHIVE 
alter column NO_BON_LIVRAISON type varchar(100);

alter table COMMANDE 
alter column TYPE_FINITION type varchar(500);

alter table COMMANDE_ARCHIVE
alter column TYPE_FINITION type varchar(500);


-- Permettre d'avoir un contact spéficique par commande 
ALTER TABLE COMMANDE 
ADD COLUMN CONTACT_PRENOM_COMMANDE varchar(50) null,
ADD COLUMN CONTACT_NOM_COMMANDE varchar(50) null,
ADD COLUMN CONTACT_TELEPHONE_COMMANDE char(10) null;

-- Idem pour la table archive
ALTER TABLE COMMANDE_ARCHIVE 
ADD COLUMN CONTACT_PRENOM_COMMANDE varchar(50) null,
ADD COLUMN CONTACT_NOM_COMMANDE varchar(50) null,
ADD COLUMN CONTACT_TELEPHONE_COMMANDE char(10) null;
...
UPDATE COMMANDE SET notes_finition = NULL WHERE notes_finition = 'None';
UPDATE COMMANDE SET notes_commande = NULL WHERE notes_commande = 'None';
UPDATE COMMANDE SET po_client_commande = NULL WHERE po_client_commande = 'None';
UPDATE COMMANDE SET type_impression = NULL WHERE type_impression = 'None';
UPDATE COMMANDE SET type_papier = NULL WHERE type_papier = 'None';
UPDATE COMMANDE SET format_final_impression = NULL WHERE format_final_impression = 'None';
UPDATE COMMANDE SET format_ouvert_impression = NULL WHERE format_ouvert_impression = 'None';